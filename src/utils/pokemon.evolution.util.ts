...
import { getPokemonItem, getPokemonMove } from "./pokemon.translate";
...
const getInitialEvoText = (detail: EvolutionDetail): string => {
    ...
    if (trigger === "level-up")
        return detail.min_level ? `nivel ${detail.min_level}` : "subir 1 nivel";
    if (trigger === "use-item") {
        const item = getPokemonItem(detail.item.name);
        return `usando ${item.name}`;
    }
    if (trigger === "trade")
        return detail.trade_species
...
export function getSingleEvolutionDetails(detail: EvolutionDetail): string {
    ...
    const heldItem = detail.held_item
        ? getPokemonItem(detail.held_item.name).name
        : null;
    ...
    const known_move = detail.known_move
        ? getPokemonMove(detail.known_move)
        : null;
    ...
    const texts = [
        ...
        known_move
            ? `conociendo el movimiento conocido ${known_move.name}`
            : "",

...
export default function Paginated({ items, list, page, className }: Props) {
    ...
    return (
        <div className="paginated paginated-module">
            <div data-items={items} data-page={initialPage} className={['paginated', className].join(' ')}>
            ...
            <div className="paginated pagination">
                <div className="pagination-wrapper">
                    <Link to={`?page=${initialPage - 1}`} className="pagination-page">
                        &laquo;
                    </Link>
                    ...
                    {initialPage > 1
                        ? Array.from({ length: 2 }, (_, i) => {
                              ...
                              return (
                                  <Link key={i} to={`?page=${page}`} className="pagination-page">
                                      {page}
                                  </Link>
                              );
                          }).reverse()
                    ...
                    <span className="pagination-page current-page">{initialPage}</span>
                    ...
                    {initialPage !== maxPages ? (
                        <Link to={`?page=${maxPages}`} className="pagination-page">
                            {maxPages}
                        </Link>
                    ) : null}
